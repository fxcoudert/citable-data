// Added by Ambroise
/*  
 * Alchemical transformation is added to build function that peform
 * an alchemical transformation at a fixed number of particule in the system
 */ 
#ifdef HAVE_CONFIG_H
#include <config.h>
#endif
...
REAL *ProtocolWorkStore;
REAL *ProtocolWorkStore_vdw;
REAL *ProtocolWorkStore_elec;
int SizeProtocolWorkStore;
...
// File to write protocol work thorugh MC.
FILE *OutputOsmostatFilePtr;

// Manage storage of the protocolwork.
void InitializeStoreProtocolWork(void)
{
	SizeProtocolWorkStore=0;
	ProtocolWorkStore=(REAL*)calloc(SizeProtocolWorkStore,sizeof(REAL));
	ProtocolWorkStore_vdw=(REAL*)calloc(SizeProtocolWorkStore,sizeof(REAL));
	ProtocolWorkStore_elec=(REAL*)calloc(SizeProtocolWorkStore,sizeof(REAL));
}

void IncreaseSizeProtocolWork(void)
{
...
void InitializeFileProtocolWork(int Size)
{	
	int i;
		
	if(ContinueAfterCrash==FALSE) 
	{
		OutputOsmostatFilePtr=fopen("ProtocolWork.txt","w");
		fprintf(OutputOsmostatFilePtr,"Widom_step\t\tProtocol_work(kJ/mol)\t\tProtocol_work_elec(kJ/mol)\t\tProtocol_work_vdw(kJ/mol)\t\tPressure (Pa)\t\tVolume (Angström^3)\n");	
		fflush(OutputOsmostatFilePtr);
	}
	else
	{
		// Read the osmostat file.
		FILE * BufferOutputOsmostatFilePtr = fopen("ProtocolWorkbuffer.txt","w");
		OutputOsmostatFilePtr=fopen("ProtocolWork.txt","r");
		...
		remove("ProtocolWork.txt");
		rename("ProtocolWorkbuffer.txt","ProtocolWork.txt");
		
		OutputOsmostatFilePtr=fopen("ProtocolWork.txt","a");
	}		
}

void UpdateFileProtocolWork(int index, FILE *FilePtr)
{
	fprintf(FilePtr,"%d\t\t\t\t\t\t%lf\t\t\t\t\t\t%lf\t\t\t\t\t\t%lf\t\t\t\t\t\t%lf\t\t\t\t\t\t%lf\n",index,
			ProtocolWorkStore[index]*ENERGY_TO_KELVIN*KELVIN_TO_KJ_PER_MOL,
			ProtocolWorkStore_elec[index]*ENERGY_TO_KELVIN*KELVIN_TO_KJ_PER_MOL,
			ProtocolWorkStore_vdw[index]*ENERGY_TO_KELVIN*KELVIN_TO_KJ_PER_MOL,
			(double)therm_baro_stats.ExternalPressure[CurrentSystem][0]*PRESSURE_CONVERSION_FACTOR,
...
void InitializeIndexManagingTransientMoities(void)
{
	...
	NumberExtraComponents = 1;
	// The number of extra pseudoatoms.
	NumberExtraPseudoAtoms = Components[SolventIndex].NumberOfAtoms;
...
void InitializeVectorCharge(void)
{
	 ...
	 VectorChargeAlchemicalTransformation = (REAL(**))calloc(2,sizeof(REAL(*)));
...
{	
    ...
    {
		for(i=0;i<InitialPseudoAtoms+TransientMoleculeNbAtoms;i++)
		{
		  for(j=i+1;j<InitialPseudoAtoms+TransientMoleculeNbAtoms;j++)
		  {
			if((i<InitialPseudoAtoms)&&(j<InitialPseudoAtoms))
			{
				continue;
